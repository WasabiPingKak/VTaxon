...
-- 12. notifications — 通知
-- ============================================================
CREATE TABLE staging.notifications (
    id SERIAL PRIMARY KEY,
    user_id UUID NOT NULL REFERENCES staging.users(id) ON DELETE CASCADE,
    type TEXT NOT NULL,
...
CREATE INDEX idx_stg_notifications_user_id ON staging.notifications(user_id);
CREATE INDEX idx_stg_notifications_user_unread ON staging.notifications(user_id, is_read) WHERE is_read = false;
CREATE INDEX idx_stg_notifications_type_ref ON staging.notifications(type, reference_id);
