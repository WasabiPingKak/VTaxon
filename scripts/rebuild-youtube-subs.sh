#!/bin/bash
# 分批重建 YouTube WebSub 訂閱
# 用法: ./scripts/rebuild-youtube-subs.sh <staging|prod> <JWT_TOKEN>
...
if [ "$ENV" = "prod" ]; then
  BASE="https://vtaxon-api-prod-135773662063.asia-east1.run.app"
...
RESULT=$(curl -s -X POST -H "Authorization: Bearer $TOKEN" \
  "$BASE/api/livestream/rebuild-youtube-subs?clean=1&offset=$OFFSET&limit=$LIMIT")
echo "$RESULT"
...
while true; do
  ...
  RESULT=$(curl -s -X POST -H "Authorization: Bearer $TOKEN" \
    "$BASE/api/livestream/rebuild-youtube-subs?offset=$OFFSET&limit=$LIMIT")
  echo "$RESULT"
  HAS_MORE=$(echo "$RESULT" | parse_has_more)
  if [ "$HAS_MORE" = "ERROR" ]; then
