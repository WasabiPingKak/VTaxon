...
import type { User, OAuthAccount, VtuberTrait, SpeciesCache } from '../types';
...
function TaxonomyPath({ trait }: TaxonomyPathProps): React.JSX.Element | null {
  ...
  const ranks = RANK_ORDER.map((rank, i) => {
    ...
    const zh = (sp as unknown as Record<string, unknown>)[`${rank}_zh`] as string | undefined;
...
function FictionalPath({ trait }: FictionalPathProps): React.JSX.Element | null {
  const f = trait.fictional_species;
  if (!f) return null;

  const levels: Array<{ rank: string; label: string }> = [
    { rank: 'F_ORIGIN', label: f.origin },
    ...(f.sub_origin ? [{ rank: 'F_SUB_ORIGIN', label: f.sub_origin }] : []),
...
export default function VTuberProfilePage(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([userData, traitsData]) => {
        if (cancelled) return;
        setUser(userData as unknown as UserWithOAuth);
  ...
  const sameAs: string[] = oauthAccounts
    .map(a => a.channel_url)
  ...
  const liveInfos: LiveInfo[] = isLive ? (liveStreams.get(userId!) || []) : [];
