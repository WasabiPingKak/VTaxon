...
export default function CharacterPage(): React.JSX.Element | null {
  ...
  const [traitVersion, bumpTraitVersion] = useReducer((c: number) => c + 1, 0);
  ...
  useEffect(() => {
    if (user) {
      // getUser returns User; oauth_accounts come from a separate endpoint
      // but the backend may include them in the response body
      api.getUser(user.id)
        .then((data) => setOauthAccounts((data as unknown as { oauth_accounts?: OAuthAccount[] }).oauth_accounts || []))
        .catch((err: Error) => console.error('Failed to load OAuth accounts:', err));
    }
  ...
  const openPreview = useCallback(async () => {
    ...
    try {
      ...
      setPreviewTraits(traitsData as unknown as Record<string, unknown>[]);
