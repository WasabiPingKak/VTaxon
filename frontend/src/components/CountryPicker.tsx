...
export default function CountryPicker({ selected = [], onChange }: CountryPickerProps): React.ReactElement {
          ...
          {filtered.map((c) => {
            ...
            return (
              <div
                ...
                onMouseEnter={(e: React.MouseEvent<HTMLDivElement>) => { if (!isSelected) e.currentTarget.style.background = 'rgba(255,255,255,0.04)'; }}
                onMouseLeave={(e: React.MouseEvent<HTMLDivElement>) => { if (!isSelected) e.currentTarget.style.background = 'transparent'; }}
              >
