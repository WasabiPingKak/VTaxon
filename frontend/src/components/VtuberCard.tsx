...
function ensureLivePulseKeyframe(): void {
...
export interface VtuberCardEntry {
  user_id: string;
  display_name: string;
...
export interface VtuberCardProps {
  entry: VtuberCardEntry;
  isCurrentUser?: boolean;
  isLive?: boolean;
  onClick?: () => void;
  activeFilterBadges?: FilterBadge[];
  sortBadge?: FilterBadge | FilterBadge[] | null;
}

const VtuberCard = memo(function VtuberCard({ entry, isCurrentUser, isLive, onClick, activeFilterBadges, sortBadge }: VtuberCardProps) {
  ...
  return (
    <button
      ...
      onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.boxShadow = isLive ? '0 2px 16px rgba(239,68,68,0.3)' : '0 2px 12px rgba(56,189,248,0.15)'; }}
      onMouseLeave={(e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.boxShadow = isLive ? '0 0 8px rgba(239,68,68,0.15)' : 'none'; }}
    >
      ...
      <div style={{ minWidth: 0 }}>
        ...
        {((activeFilterBadges && activeFilterBadges.length > 0) || sortBadge) && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px', marginTop: '2px', alignItems: 'center' }}>
            ...
            {sortBadge && ([] as FilterBadge[]).concat(sortBadge as FilterBadge | FilterBadge[]).map((b, i) =>
              b.isCountry && b.countryCode ? (
                <span key={`s${i}`} style={{
                  padding: '1px 3px', borderRadius: 3, background: b.bg,
                  display: 'inline-flex', alignItems: 'center',
