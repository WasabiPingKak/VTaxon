...
interface Facets {
  ...
  has_traits?: Record<string, number>;
}
...
export default function DirectoryFilters({
  ...
  const updateSetFilter = (key: string, newSet: Set<string>) => {
    updateFilter(key, newSet.size > 0 ? [...newSet].join(',') : '');
  };

  const toSet = (str: string | undefined): Set<string> => new Set(str ? str.split(',').filter(Boolean) : []);
  ...
  interface Chip { key: string; value: string; label: string; flagCode?: string; }
  const chips: Chip[] = [];
  ...
  const removeChip = (chip: Chip) => {
    if (chip.key === 'org_type') {
      ...
      const set = toSet(filters[chip.key] as string | undefined);
...
function SortControls({ filters, updateFilter }: SortControlsProps) {
