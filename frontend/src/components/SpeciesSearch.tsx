...
interface InterceptSpecies extends SpeciesResult {
  _breeds: Breed[];
  _speciesInfo: SpeciesResult;
}

interface SpeciesGroupData {
  speciesKey: number;
  species: SpeciesResult | null;
  subspecies: SpeciesResult[];
  highRank?: boolean;
}
...
interface BreedRowProps {
  sp: SpeciesResult;
  onSelect?: (sp: SpeciesResult) => void;
}

/** Breed result row */
function BreedRow({ sp, onSelect }: BreedRowProps) {
  const breed = sp.breed || {} as Breed;
...
function groupBySpecies(results: SpeciesResult[]): { breeds: SpeciesResult[]; speciesGroups: SpeciesGroupData[] } {
  const breeds: SpeciesResult[] = [];
  const groups = new Map<number, SpeciesGroupData>();
...
/** Build breadcrumb */
function Breadcrumb({ sp }: BreadcrumbProps) {
  const ranks = [
...
function SpeciesRow({ sp, onSelect, indent, connector, familyColor }: SpeciesRowProps) {
  ...
  const rank = (sp.taxon_rank || '').toUpperCase();
  const altFull = formatAltNamesFull(sp.alternative_names_zh as string | null | undefined);
  const altTitle = altNamesTooltip(sp.alternative_names_zh as string | null | undefined);
...
interface SpeciesGroupProps {
  group: SpeciesGroupData;
  onSelect?: (sp: SpeciesResult) => void;
  familyColor?: string;
}

function SpeciesGroup({ group, onSelect, familyColor }: SpeciesGroupProps) {
      ...
      try {
        ...
        await api.getSubspeciesStream(speciesKey, (sub) => {
          if (!existing.has(sub.taxon_id)) {
            existing.set(sub.taxon_id, sub as unknown as SpeciesResult);
...
export interface SpeciesSearchProps {
  ...
  onCancel?: () => void;
  autoFocus?: boolean;
  onSearchPerformed?: (info: { query: string; resultCount: number }) => void;
}
...
export default function SpeciesSearch({ onSelect, onCancel, autoFocus, onSearchPerformed }: SpeciesSearchProps) {
  ...
  const [interceptSpecies, setInterceptSpecies] = useState<InterceptSpecies | null>(null);
  ...
  function markLatinIfApplicable(q: string) {
    if (!CJK_RE.test(q)) {
      setHasSearchedLatin(true);
    }
  }

  async function handleSpeciesSelect(species: SpeciesResult) {
    ...
    try {
      const data = await api.getBreeds(species.taxon_id) as unknown as { breeds?: Breed[]; species?: SpeciesResult };
      if (data.breeds?.length) {
        setInterceptSpecies({ ...species, _breeds: data.breeds, _speciesInfo: (data.species || species) as SpeciesResult });
        return;
      }
  ...
  async function handleSearch(e: React.FormEvent) {
    ...
    try {
      await api.searchSpeciesStream(query, (sp) => {
        count++;
        setResults(prev => [...prev, sp as unknown as SpeciesResult]);
      }, { signal: ac.signal });
    } catch (err) {
      if ((err as Error).name === 'AbortError') return;
  ...
  if (interceptSpecies) {
    return (
      <div>
        <BreedInterceptPanel
          species={interceptSpecies as Parameters<typeof BreedInterceptPanel>[0]['species']}
          onSelectBreed={(payload) => {
            setInterceptSpecies(null);
            onSelect(payload as unknown as SpeciesResult);
          }}
          onSkip={() => {
            const sp = interceptSpecies;
            setInterceptSpecies(null);
            // Pass original species without breed
            const { _breeds: _, _speciesInfo: __, ...clean } = sp;
            onSelect(clean);
          }}
