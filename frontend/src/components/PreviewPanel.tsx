...
function TaxonomyPath({ species }: TaxonomyPathProps) {
  ...
  const ranks = RANK_ORDER.map((rank, i) => {
    ...
    const zh = species[`${rank}_zh`] as string | undefined;
...
function FictionalPath({ fictional }: FictionalPathProps) {
  const levels = [
  ...
  ].filter(Boolean) as Array<{ rank: string; label: string }>;
...
export interface PreviewPanelProps {
  ...
  traits: TraitWithRelations[];
  selectedTraitIdx: number;
  onSelectTrait: (idx: number) => void;
  onClose: () => void;
}
          ...
          <div style={{
            ...
            {allTraits.map((t, i) => {
              const label = t.taxon_id
                ? (t.species?.common_name_zh || displayScientificName(t.species as SpeciesData) || t.display_name)
              ...
              <div style={{ fontSize: '0.9em', lineHeight: '1.8' }}>
                ...
                {species.alternative_names_zh && (
                  <div>
                    ...
                    {(species.alternative_names_zh as string).split(/[,，]/).map(s => s.trim()).filter(Boolean).join('、')}
                  </div>
                )}
                {trait.breed_name && (
                  <div>
