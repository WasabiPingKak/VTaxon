...
import type { TreeFilters, TreeFacets } from '../../types/tree';

const GENDER_LABELS: Record<string, string> = { '男': '男', '女': '女', 'other': '自訂', unset: '未設定' };
const STATUS_LABELS: Record<string, string> = { active: '活動中', hiatus: '活動休止', preparing: '準備中', unset: '未設定' };
const ORG_LABELS: Record<string, string> = { corporate: '企業勢', indie: '個人勢', club: '社團勢' };
const PLATFORM_LABELS: Record<string, string> = { youtube: 'YouTube', twitch: 'Twitch' };
 ...
 */
export default function FilterPanel({ filters, onFiltersChange, facets, onClose, isMobile, open: openProp }: FilterPanelProps) {
  ...
  useEffect(() => {
    ...
    const handler = (e: PointerEvent) => {
      if (panelRef.current && !panelRef.current.contains(e.target as Node)
          && !(e.target as HTMLElement).closest('[data-filter-toggle]')) onClose();
    };
  ...
  const toggle = (dim: keyof TreeFilters, val: string) => {
  ...
  const toggleSingle = (dim: keyof TreeFilters, val: string) => {
    const set = new Set(filters[dim]);
    if (set.has(val)) set.delete(val);
  ...
  // Build options
  const countryOpts = useMemo((): FilterOption[] => {
    if (!facets?.country) return [];
    let noneItem: FilterOption | null = null;
  ...
  const genderOpts = useMemo((): FilterOption[] => {
  ...
  const orgOpts = useMemo((): FilterOption[] => {
    if (!facets?.org_type) return [];
    return [...facets.org_type.entries()].map(([v, c]) => ({ value: v, label: ORG_LABELS[v] || v, count: c }));
  }, [facets?.org_type]);

  const platformOpts = useMemo((): FilterOption[] => {
    if (!facets?.platform) return [];
    return [...facets.platform.entries()].map(([v, c]) => ({ value: v, label: PLATFORM_LABELS[v] || v, count: c }));
  }, [facets?.platform]);

  const sections = ([
    { key: 'country' as const, title: '國旗', options: countryOpts, multi: true },
    { key: 'gender' as const, title: '性別', options: genderOpts, multi: true },
    { key: 'status' as const, title: '狀態', options: statusOpts, multi: true },
    { key: 'org_type' as const, title: '組織', options: orgOpts, multi: false },
    { key: 'platform' as const, title: '平台', options: platformOpts, multi: true },
  ] satisfies FilterSection[]).filter(s => s.options.length > 0);

  const checkboxRow = (dim: keyof TreeFilters, opt: FilterOption, multi: boolean): ReactNode => {
    const selected = filters[dim] || new Set<string>();
