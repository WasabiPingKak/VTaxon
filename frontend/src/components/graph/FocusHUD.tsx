...
const getSpeciesLabel = (entry: TreeEntry): string =>
  entry.fictional_name_zh
  || entry.breed_name_zh || entry.breed_name
  || entry.common_name_zh || displayScientificName(entry as Parameters<typeof displayScientificName>[0]) || '?';

interface FocusHUDProps {
  focusedEntries: TreeEntry[] | null;
  speciesIndex: number;
  onPrev: () => void;
  onNext: () => void;
  onLocate: () => void;
  onJumpToSpecies?: (index: number) => void;
  isMobile: boolean;
}
 ...
 */
export default function FocusHUD({ focusedEntries, speciesIndex, onPrev, onNext, onLocate, onJumpToSpecies, isMobile }: FocusHUDProps) {
