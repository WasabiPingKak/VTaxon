...
import type { TreeFilters, SortKey, TreeEntry } from '../../types/tree';
import {
...
function drawPlatformIcon(ctx: CanvasRenderingContext2D, platform: string, x: number, y: number, iconSize: number): void {
...
function collapsedRectWeight(count: number | undefined, maxCount: number | undefined): number {
...
function isInViewport(x: number, y: number, vp: Viewport, margin: number): boolean {
...
function scaledFontSize(basePx: number, scale: number): number {
  return basePx / Math.max(scale, FONT_MIN_SCALE);
}

function fontStr(basePx: number, scale: number, weight = '', style = ''): string {
...
function drawWrappedText(ctx: CanvasRenderingContext2D, lines: string[], x: number, startY: number, lineHeight: number): void {
...
function edgeFlashAlpha(state: DrawGraphState): number {
...
function drawBreedGridConnectors(
  ctx: CanvasRenderingContext2D,
  nodes: LayoutNode[],
  scale: number,
  pm: Map<string, AnimatedPosition> | null | undefined,
...
function drawBudgetBadge(ctx: CanvasRenderingContext2D, node: LayoutNode, scale: number, bottomY: number, state: DrawGraphState): void {
  const d = node.data;
...
function drawTaxonomyNode(ctx: CanvasRenderingContext2D, node: LayoutNode, scale: number, state: DrawGraphState): void {
  const d = node.data;
...
function drawSpeciesNode(ctx: CanvasRenderingContext2D, node: LayoutNode, scale: number, state: DrawGraphState): void {
  const d = node.data;
...
function drawVtuberNode(ctx: CanvasRenderingContext2D, node: LayoutNode, scale: number, state: DrawGraphState): void {
  const d = node.data;
...
function drawDotVtuberNode(ctx: CanvasRenderingContext2D, node: LayoutNode, scale: number, state: DrawGraphState): void {
  const d = node.data;
  const dotR = 5;
...
function drawBreedNode(ctx: CanvasRenderingContext2D, node: LayoutNode, scale: number, state: DrawGraphState): void {
  const d = node.data;
...
// ── Hex ring helper ──
function drawHexRing(ctx: CanvasRenderingContext2D, x: number, y: number, radius: number, strokeStyle: string, lineWidth: number): void {
  ctx.beginPath();
  for (let i = 0; i < 6; i++) {
    const angle = (Math.PI / 3) * i - Math.PI / 6;
