...
export default function ProfileInfoCard({ profileData }) {
  ...
  const status = pd.activity_status && STATUS_MAP[pd.activity_status];
  ...
  const hasCreators = (pd.illustrators?.length > 0) || (pd.riggers?.length > 0) || (pd.modelers_3d?.length > 0);
  const hasHashtags = pd.hashtags?.length > 0;
