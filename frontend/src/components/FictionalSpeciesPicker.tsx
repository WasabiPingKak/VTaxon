...
import type { FictionalSpecies } from '../types';
...
const SUB_ORIGIN_ORDER: Record<string, string[]> = {
  '東方神話': ['中國神話', '日本神話', '台灣民間傳說'],
  '西方神話': ['希臘神話', '北歐神話', '歐洲民間傳說', '凱爾特神話', '日耳曼傳說', '埃及神話'],
  '奇幻文學': ['通用'],
  '人造生命': ['機械生命', '生物合成', '附魂體'],
...
function sortedEntries<K extends string | null, V>(map: Map<K, V>, orderArr: string[]): Array<[K, V]> {
  const rank = new Map<string | null, number>(orderArr.map((k, i) => [k, i]));
...
interface TypeData {
  typeNode: FictionalSpecies | null;
  children: FictionalSpecies[];
}

interface SubGroup {
  types: Map<string, TypeData>;
  directSpecies: FictionalSpecies[];
}

export interface FictionalSpeciesPickerProps {
  existingTraitIds?: number[];
  onAdd: (species: FictionalSpecies) => Promise<void>;
}

export default function FictionalSpeciesPicker({ existingTraitIds = [], onAdd }: FictionalSpeciesPickerProps) {
  const [allSpecies, setAllSpecies] = useState<FictionalSpecies[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await api.getFictionalSpecies() as unknown as { species?: FictionalSpecies[] };
        if (!cancelled) setAllSpecies((data as { species?: FictionalSpecies[] }).species || (data as unknown as FictionalSpecies[]));
      } catch (err) {
  ...
  const globalTypeNamesPerSub = useMemo(() => {
    const m = new Map<string, Set<string>>(); // key: "origin|sub_origin" -> Set of type names
  ...
  const typeNodeByPath = useMemo(() => {
    const m = new Map<string, FictionalSpecies>();
    for (const sp of allSpecies) {
      if (!sp.category_path) continue;
      const segments = sp.category_path.split('|');
  ...
  const grouped = useMemo(() => {
    const map = new Map<string, Map<string | null, SubGroup>>();
...
function SubContent({ subKey, group, existingSet, onAdd, isSearching, expandedTypes, toggleType }: SubContentProps) {
  return (
    <>
      {/* Type rows */}
      {Array.from(group.types.entries()).map(([typeName, typeData]) => (
        <TypeRow
          ...
          typeChildren={typeData.children}
          existingSet={existingSet}
          onAdd={onAdd}
          isSearching={isSearching}
...
function TypeRow({ typeKey: _typeKey, typeName, typeNode, typeChildren, existingSet, onAdd, isSearching: _isSearching, expanded, onToggle }: TypeRowProps) {
...
interface SpeciesItemProps {
  species: FictionalSpecies;
  isAdded: boolean;
  onAdd: (species: FictionalSpecies) => Promise<void>;
}

function SpeciesItem({ species, isAdded, onAdd }: SpeciesItemProps) {
  const [adding, setAdding] = useState(false);
