...
import type { TaxonomyTreeNode, TreeEntry, TreeFilters } from '../types';
...
const emptyArrowStyle: React.CSSProperties = { display: 'inline-block', width: '16px' };
...
const dotCardStyle: React.CSSProperties = {
...
interface VtuberTierGroupProps {
  vtubers: TreeEntry[];
  depth: number;
  currentUserId: string | undefined;
  liveUserIds: Set<string> | undefined;
  onSelectVtuber: (v: TreeEntry) => void;
  activeFilters: TreeFilters | undefined;
  sortKey: string | undefined;
}

function VtuberTierGroup({ vtubers, depth, currentUserId, liveUserIds, onSelectVtuber, activeFilters, sortKey }: VtuberTierGroupProps) {
  const [budgetExpanded, setBudgetExpanded] = useState(false);

  const normal: TreeEntry[] = [];
  const dot: TreeEntry[] = [];
...
function setsEqualForNode(a: Set<string> | undefined, b: Set<string> | undefined, pathKey: string, isExpanded: boolean): boolean {
  if (a === b) return true;
  if (!a || !b) return a === b;
...
const TreeNode = memo(function TreeNode({
  ...
  const visibleChildren: TaxonomyTreeNode[] = [];
  ...
  let computedCountBadgeStyle: React.CSSProperties = countBadgeStyle;
