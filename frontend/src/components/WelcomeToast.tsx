...
function ToastCard({ user, onClose, compact }: ToastCardProps) {
  ...
  return (
    ...
    >
      ...
      <button
        ...
        onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => e.currentTarget.style.color = 'rgba(255,255,255,0.8)'}
        onMouseLeave={(e: React.MouseEvent<HTMLButtonElement>) => e.currentTarget.style.color = 'rgba(255,255,255,0.35)'}
      >
        ✕
      </button>
    </div>
...
export default function WelcomeToast({ onNewUsers, visible = true }: WelcomeToastProps) {
  const [toasts, setToasts] = useState<RecentUser[]>([]);
  const [mobileExpanded, setMobileExpanded] = useState(false);
  const queueRef = useRef<RecentUser[]>([]);
  ...
  const fetchRecent = useCallback(async (since: string) => {
    try {
      const users = await api.getRecentUsers(since) as unknown as RecentUser[];
  ...
  const dismissToast = useCallback((userId: string) => {
        ...
        {displayToasts.map(user => (
          <div key={user.id} style={{ pointerEvents: 'auto', position: 'relative' }}>
            ...
            {showCollapsedMobile && user.id === latestToast.id && (
              <button
                onClick={(e: React.MouseEvent) => { e.stopPropagation(); setMobileExpanded(true); }}
                style={{
                  position: 'absolute',
                  top: -8,
                  left: -8,
        ...
        <div style={{ pointerEvents: 'auto', display: 'flex', justifyContent: 'flex-end', gap: 6 }}>
          {isMobile && mobileExpanded && (
            <button
              ...
              onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.color = 'rgba(255,255,255,0.85)'; e.currentTarget.style.background = 'rgba(20,28,40,0.95)'; }}
              onMouseLeave={(e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.color = 'rgba(255,255,255,0.5)'; e.currentTarget.style.background = 'rgba(20,28,40,0.8)'; }}
            >
              收合
            </button>
          )}
