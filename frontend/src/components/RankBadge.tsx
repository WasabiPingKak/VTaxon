...
const RANK_CONFIG: Record<string, RankConfigEntry> = {
  ...
  FORM:       { label: '變型', bg: 'rgba(16,185,129,0.12)', color: '#10b981', border: 'rgba(16,185,129,0.25)' },
...
export function getRankConfig(rank: string): RankConfigEntry | null {
  return RANK_CONFIG[(rank || '').toUpperCase()] || null;
}

export function getRankLabel(rank: string): string | null {
...
export default function RankBadge({ rank, style: extraStyle }: RankBadgeProps): React.ReactElement | null {
