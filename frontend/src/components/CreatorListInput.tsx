...
interface CreatorListInputProps {
  value?: CreatorItem[];
  onChange: (items: CreatorItem[]) => void;
}

export default function CreatorListInput({ value = [], onChange }: CreatorListInputProps): React.ReactElement {
  const update = useCallback((idx: number, field: keyof CreatorItem, v: string) => {
      ...
      {value.map((item, i) => (
        <div key={i} style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
          ...
          <input
            type="text"
            value={item.url || ''}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => update(i, 'url', e.target.value)}
            placeholder="連結（選填）"
            autoComplete="new-password"
