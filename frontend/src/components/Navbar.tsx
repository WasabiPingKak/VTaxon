...
function AvatarFallback({ name, size = 32 }: AvatarFallbackProps) {
  ...
  useEffect(() => {
    ...
    const fetchCounts = () => {
      api.getAdminCounts()
        .then(data => {
          if (cancelled) return;
          const d = data as unknown as Record<string, Record<string, number>>;
          const fc = d.fictional || {};
          const bc = d.breed || {};
          const rc = d.report || {};
          ...
          user ? (
            <>
              <button
                ...
                onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.borderColor = 'rgba(233,30,140,0.6)'; e.currentTarget.style.background = 'rgba(233,30,140,0.18)'; }}
                onMouseLeave={(e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.borderColor = 'rgba(233,30,140,0.3)'; e.currentTarget.style.background = 'rgba(233,30,140,0.1)'; }}
              >
