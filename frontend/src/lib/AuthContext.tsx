...
import type { User, OAuthAccount } from '../types/models';
...
interface AuthContextValue {
  ...
  linkProvider: (provider: 'google' | 'twitch') => Promise<void>;
  unlinkProvider: (identityId: string) => Promise<void>;
  ytPermissionFailed: boolean;
}

const AuthContext = createContext<AuthContextValue | null>(null);

const PROVIDER_LABELS: Record<string, string> = { google: 'YouTube', youtube: 'YouTube', twitch: 'Twitch' };
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [ytPermissionModal, setYtPermissionModal] = useState<boolean>(false);
  ...
  async function syncUser(session: Session): Promise<void> {
    ...
    try {
      ...
      const displayName = ytChannel?.channelTitle
        || (isTwitchLogin ? ((meta as Record<string, string>).nickname || (meta as Record<string, string>).slug || (meta as Record<string, string>).name) : ((meta as Record<string, string>).full_name || (meta as Record<string, string>).name))
        || 'Unnamed Vtuber';
      const avatarUrl = ytChannel?.channelAvatar
        || (meta as Record<string, string>).avatar_url || (meta as Record<string, string>).picture;
      ...
      try {
        ...
        if ((err as ApiError).data?.error === 'account_banned') {
          if (!mountedRef.current) return;
      ...
      if (identities.length > 0) {
        const syncBody: Record<string, unknown> = { identities, create_missing: isFreshOAuth };
      ...
      const providerMap: Record<string, string> = { google: 'youtube', twitch: 'twitch' };
      if (loginProvider && identities.length > 1) {
        try {
          const boundAccounts: OAuthAccount[] = await api.getMyOAuthAccounts();
          ...
          const unbound = identities
            ...
            .filter(p => !boundProviders.has(p as OAuthAccount['provider']));
          if (unbound.length > 0) {
  ...
  async function linkProvider(provider: 'google' | 'twitch'): Promise<void> {
    ...
    if (user) {
      try {
        ...
        sessionStorage.setItem('vtaxon_pending_link', token);
      } catch (err) {
  ...
  async function unlinkProvider(identityId: string): Promise<void> {
    ...
    try {
      ...
      console.warn('unlinkIdentity not available:', (err as Error).message);
    }
  }
