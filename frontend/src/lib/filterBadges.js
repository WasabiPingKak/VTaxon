...
export function getSortBadge(entry, sortKey) {
  ...
  if (sortKey === 'created_at') {
    ...
    else if (diffDays < 30) label = `${diffDays}天前`;
    else if (diffDays < 365) label = `${Math.floor(diffDays / 30)}月前`;
    else label = `${Math.floor(diffDays / 365)}年前`;
    return { label, color: '#67e8f9', bg: 'rgba(103,232,249,0.15)' };
  }
