...
| Testing      | pytest (backend, 546 tests)                                         |
...
目前後端共 546 個測試案例（行覆蓋率 70%），涵蓋 JWT/JWKS 驗證、權限裝飾器、GBIF 物種搜尋、中文名稱 fallback、OAuth、Webhooks、SSR、外部 API 整合（Twitch/YouTube/Wikidata/TaiCOL）、moderation 等核心邏輯。CI pipeline 會在每次部署前自動執行。
...
│   │   ├── services/           # 業務邏輯
│   │   │   ├── admin.py        # 管理後台邏輯
│   │   │   ├── alerts.py       # Alert Digest 告警系統
│   │   │   ├── chinese_names.py # 中文名稱整合查詢
│   │   │   ├── directory.py    # VTuber 目錄查詢
