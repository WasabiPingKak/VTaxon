...
class TestBuildTaxonPath:
    ...
    def test_missing_middle_ranks(self, app):
        ...
        with app.app_context():
            ...
            data = {
                'kingdom': 'Animalia', 'phylum': 'Chordata',
                'family': 'Arandaspididae', 'genus': 'Sacabambaspis',
            }
            path = _build_taxon_path(data)
    ...
    def test_resolves_accepted(self, app):
        with app.app_context():
            ...
            resp2.json.return_value = {
                'key': 200, 'scientificName': 'Accepted sp.',
    ...
    def test_taicol_fallback(self, app):
        with app.app_context():
            ...
            with patch('app.services.gbif.get_species_zh_override', return_value=None):
                with patch('app.services.gbif.taicol_get_chinese_name',
                           return_value=('家貓', None)):
                    result = _resolve_chinese_name(9685, 'Felis catus')
                    assert result == '家貓'
        ...
        with app.app_context():
            ...
            with patch('app.services.gbif.get_species_zh_override', return_value=None):
                with patch('app.services.gbif.taicol_get_chinese_name',
                    ...
                    with patch('app.services.gbif.get_chinese_name_by_gbif_id',
                               return_value=('貓', 'Cat')):
                        result = _resolve_chinese_name(9685, 'Felis catus')
                        assert result == '貓'
...
class TestSuggestSpecies:
    ...
    def test_filters_accepted_species(self, app):
        with app.app_context():
            ...
            mock_resp.json.return_value = [
                {
                    'key': 1, 'status': 'ACCEPTED', 'rank': 'SPECIES',
                    'scientificName': 'Felis catus', 'canonicalName': 'Felis catus',
                    'kingdom': 'Animalia', 'phylum': 'Chordata',
                    'class': 'Mammalia', 'order': 'Carnivora',
                    'family': 'Felidae', 'genus': 'Felis',
                    'species': 'Felis catus',
                ...
                {
                    'key': 2, 'status': 'ACCEPTED', 'rank': 'UNRANKED',
    ...
    def test_deduplicates_by_key(self, app):
        with app.app_context():
            ...
            mock_resp.json.return_value = [
                {
                    'key': 1, 'status': 'ACCEPTED', 'rank': 'SPECIES',
                    'scientificName': 'Sp A', 'canonicalName': 'Sp A',
                    'kingdom': 'Animalia',
                },
                {
                    'key': 1, 'status': 'ACCEPTED', 'rank': 'SPECIES',
...
class TestSearchSpecies:
    ...
    def test_cjk_routes_to_local_and_taicol(self, app):
        with app.app_context():
            ...
            with patch('app.services.gbif._search_breeds', return_value=[]):
                with patch('app.services.gbif._search_local_cache_chinese',
                           return_value=[local_result]):
                    with patch('app.services.gbif._search_via_taicol',
                               return_value=[taicol_result]):
    ...
    def test_latin_routes_to_suggest(self, app):
        with app.app_context():
            ...
            with patch('app.services.gbif._search_breeds', return_value=[]):
                with patch('app.services.gbif.suggest_species',
